...
└─ extension-firefox/        расширение (MV2, persistent background)
   ...
   ├─ content.js             поллинг композера + клики кнопок диктовки + сигнал recording + очистка + register таба
   ├─ background.js          WS-клиент с реконнектом, форвард команд в табы, активация таба ChatGPT на mic (§6.15), индикатор
